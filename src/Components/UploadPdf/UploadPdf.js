...
const UploadPdf = () => {
  ...
  const [textFileBox, setTextFileBox]=useState(null);

  //const [progressValue, setProgressValue] = useState(0); // Initialize progress state

  const { loading, progress, error, success } = useSelector(
    (state) => state.pdfs
  );
  ...
  useEffect(() => {
    ...
    if (progress === 100) {
      setShowAlert(true);
      setAlertMessage(
        `The File: ${pdfFile.name} has been successfulled Upload`
      );
      setReloadFile(true);
    }
    ...
    if (reloadFile) {
      if (progress >= 100) {
        if(textFileBox!=null){
          dispatch(downloadPdf(textFileBox)); 
          setTextFileBox(null);
          //setPdfFile(null);
          setReloadFile(false);
        }
  ...
  }, [dispatch, progress, success, error, pdfFile, loading, reloadFile, textFileBox]);
